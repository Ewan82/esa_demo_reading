c**************************************************************
c  Name : nadimbrf_energie.f
c  additional routine by tristan quaife for calling the energie
c  subroutine at the end of the brf routine to avoid horrible
c  global variable when calling from C
c***************************************************************
      SUBROUTINE nadimbrfe(THETA_I,PHI_I,NV,THETA_V,PHI_V,
     * LAD,XRS,XHC,XLAI,RPL,XRL,XTL,BRF, FPAR, ALBEDO_SYS, TRANS_TOTAL)
c
      implicit none
      INTEGER LAD,NV
      REAL BRF(NV)
...
c
      ...
      BRF(i)= xmeas
			...
			call energie(theta_i,phi_i,fpar,albedo_sys,trans_total)
