...
program runretrieval
  ...
  real(kind=8) :: pgtol
  !--
  logical :: run_targets !-- whether to apply the target operators for prior/post
                         !   (including uncertainty propagation)
  integer :: ldebug
  logical :: tstflg
  integer :: ierr, iostat
  logical :: diag
  ...
  if(narg>0)then
     ...
     arg: do iarg=1,narg
        ...
        case("--help","-h")
           ...
           option = '--dbglev'
           write(*,fmt=3333) option, 'increase level of verbosity'
           option = '--no_targets'
  ...
  write(*, '(a,f6.2)') ' INFO::runretrieval:'//&
       'will apply relative perturbation of prior with pert=',pert
  ...
  if( run_targets ) then
     write(*, '(a)') ' INFO::runretrieval:'//&
          'start HH-backscatter target processing...'
     call backscatter_hh_retrieval(n, x, cx)
     write(*, '(a)') ' INFO::runretrieval:'//&
          '...HH-backscatter target processing DONE'

     write(*, '(a)') ' INFO::runretrieval:'//&
          'start FAPAR target processing...'
     call fapar_retrieval(n, x, cx)
     write(*, '(a)') ' INFO::runretrieval:'//&
          '...FAPAR target processing DONE'
  endif
