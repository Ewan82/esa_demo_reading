...
!> \brief provides implementation of target operators in the
...
!     simulate_fapar
!
!> @brief FAPAR simulation for a complete control vector
!
!
...
!     simulate_bscat_hh
!
!> @brief Backscatter values in HH polarisation will be computed for the
...
!                   by state variables LAI,HC,SM per 'simulation point')
!> @param[in]   m  length of output vector
!> @param[out]  y  simulated backscatter values (HH)
!
subroutine simulate_bscat_hh(n, x, m, y)
  use mo_sensimul, only:nparam_s1, nsc, get_n,get_np,get_npts
  use mo_sensimul, only:iv_geom, idx_is_s1, sim_fill_value
  implicit none
  ! arguments
  integer, intent(in) :: n, m
  real(kind=8), intent(in) :: x(n)
  real(kind=8), intent(out) :: y(m)
  ! externals
  external simulate_bscat_hh_1geom
  ! local decls
  integer :: ipt
  integer :: i0,i1
  real(kind=8) :: xphys(n)
  ...
  real(kind=8) :: single_ivgeom(4)
  real(kind=8) :: bscat(3) !-- vh,vv,hh
  !-- dimension consistency
  if( n.ne.get_n() ) then
     write(*, '(a,2(a,i3,1x))') ' FATAL::simulate_bscat_hh:inconsistent length of state vector.',&
          'expected=',get_n(),'got=',n
     stop
  endif
  if( m.ne.get_npts() ) then
     write(*, '(a,2(a,i3,1x))') ' FATAL::simulate_bscat_hh:inconsistent length of result vector.',&
          'expected=',get_npts(),'got=',m
  endif
  ...
  !-- loop overa all time-points
  i0 = get_np() + 1 !-- state-vector components start *after* parameter
  simloop: do ipt=1,m
     if( .not.idx_is_s1(ipt) ) then !-- no S1 acquisition time => no 'HH' will be simulated
        i0 = i0 + nsc
     else
        !-- extract 'nsc' state vector components
        !   for current time-point
        i1 = i0 + nsc - 1
        !-- set control for current time-point
        statev(nparam_s1:nparam_s1+nsc) = xphys(i0:i1)
        !-- get gemoetry
        single_ivgeom(1:4) = iv_geom(1:4,ipt)
        !-- call HH target operator
        call allbackscatter_operator_1geom( statev, single_ivgeom, bscat)
        !-- take 'hh' only
        y(ipt) = bscat(3)
        !-- increment start position for next time-point in control vector
        i0 = i1 + 1
     endif
  end do simloop

end subroutine simulate_bscat_hh
