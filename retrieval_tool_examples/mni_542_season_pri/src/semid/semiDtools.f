...
C
      ...
      real teta_i,teta_e
      integer number,ild
      real, external :: psi_leaf_p, psi_leaf_m, gl_bun
      integer j
      real x
      common/ga/weights(32),points(32)
      common/feuille/tl,rl,lai
...
C
      ...
      real tl,rl
      real df, a_f, h_c, r
      real x_lambda_trou, x_r1, x_r2, a_trou, x_trou
      common/feuille/tl,rl,lai
...
C
C     iLab-changed:xIf(m+1,?) will be accessed later and *must*
C                  be initialised (21 is the size of first dimension in the declaraion)
      do k=1,min(m+1,21)
      do j=1,number
      S(k,j)=0.
      xIf(k,j)=0.
...
c
      ...
      xl=0.5_8*(x2-x1)
      do 12 i=1,m
      z=cos(3.141592654_8*(i-.25_8)/(n+.5_8))
1     continue
      p1=1._8
      p2=0._8
      do 11 j=1,n
      p3=p2
      p2=p1
      p1=((2._8*j-1._8)*z*p2-(j-1._8)*p3)/j
11    continue
      pp=n*(z*p1-p2)/(z*z-1._8)
      z1=z
      z=z1-p1/pp
      if(abs(z-z1).gt.eps)go to 1
      x(i)=xm-xl*z
      x(n+1-i)=xm+xl*z
      w(i)=2._8*xl/((1._8-z*z)*pp*pp)
      w(n+1-i)=w(i)
12    continue
      return
      end
