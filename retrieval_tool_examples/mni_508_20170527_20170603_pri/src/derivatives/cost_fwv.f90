...
!  Differentiation of simulate_bscat_hh in forward (tangent) mode (with options messagesInFile noinclude noISIZE r8 multiDirectio
!nal):
!   variations   of useful results: y
!   with respect to varying inputs: x
!   RW status of diff variables: x:in y:out
...
!
SUBROUTINE SIMULATE_BSCAT_HH_FWV(n, x, x_fwv, m, y, y_fwv, nbdirs)
...
!-- vh,vv,hh
  REAL(kind=8) :: bscat(3)
  REAL(kind=8) :: bscat_fwv(nbdirsmax, 3)
  INTEGER :: nd
  INTEGER :: nbdirs
!-- dimension consistency
  IF (n .NE. GET_N()) THEN
    WRITE(*, '(a,2(a,i3,1x))') &
&   ' FATAL::simulate_bscat_hh:inconsistent length of state vector.', &
&   'expected=', GET_N(), 'got=', n
    STOP
  ELSE
    IF (m .NE. GET_NPTS()) WRITE(*, '(a,2(a,i3,1x))') &
&      ' FATAL::simulate_bscat_hh:inconsistent length of result vector.'&
&                          , 'expected=', GET_NPTS(), 'got=', m
!-- initialise
    y(1:m) = sim_fill_value
!-- get physical control vector
...
!-- state-vector components start *after* parameter
    ...
    END DO
simloop:DO ipt=1,m
      IF (.NOT.IDX_IS_S1(ipt)) THEN
!-- no S1 acquisition time => no 'HH' will be simulated
        i0 = i0 + nsc
      ELSE
!-- extract 'nsc' state vector components
!   for current time-point
...
!-- set control for current time-point
          statev_fwv(nd, nparam_s1:nparam_s1+nsc) = xphys_fwv(nd, i0:i1)
        END DO
        statev(nparam_s1:nparam_s1+nsc) = xphys(i0:i1)
!-- get gemoetry
        single_ivgeom(1:4) = iv_geom(1:4, ipt)
!-- call HH target operator
        CALL ALLBACKSCATTER_OPERATOR_1GEOM_FWV(statev, statev_fwv, &
&                                        single_ivgeom, bscat, bscat_fwv&
&                                        , nbdirs)
        DO nd=1,nbdirs
!-- take 'hh' only
          y_fwv(nd, ipt) = bscat_fwv(nd, 3)
        END DO
        y(ipt) = bscat(3)
!-- increment start position for next time-point in control vector
    ...
    END DO simloop
  END IF
END SUBROUTINE SIMULATE_BSCAT_HH_FWV

!  Differentiation of allbackscatter_operator_1geom in forward (tangent) mode (with options messagesInFile noinclude noISIZE r8 m
!ultiDirectional):
...
!
SUBROUTINE ALLBACKSCATTER_OPERATOR_1GEOM_FWV(statev, statev_fwv, iv_geom&
& , bscat, bscat_fwv, nbdirs)
...
!sza,saa,vza,vaa
  REAL(kind=8), INTENT(IN) :: iv_geom(4)
!VH,VV,HH
  REAL(kind=8), INTENT(OUT) :: bscat(3)
  REAL(kind=8), INTENT(OUT) :: bscat_fwv(nbdirsmax, 3)
! externals
!core SENSE routine
...
& sm_fwv
  ...
  REAL(kind=8), DIMENSION(nbdirsmax) :: s0vh_fwv, s0vv_fwv, s0hh_fwv
...
!-- call SENSE core routine
  CALL SIGMA0_VHVVHH_FWV(lai, lai_fwv, lai_coeff, lai_coeff_fwv, hc, &
&                  hc_fwv, sm, sm_fwv, s0vh, s0vh_fwv, s0vv, s0vv_fwv, &
&                  s0hh, s0hh_fwv, nbdirs)
  DO nd=1,nbdirs
!-- map backscatter values
    bscat_fwv(nd, 1) = s0vh_fwv(nd)
    bscat_fwv(nd, 2) = s0vv_fwv(nd)
    bscat_fwv(nd, 3) = s0hh_fwv(nd)
  END DO
  bscat(1) = s0vh
  bscat(2) = s0vv
  bscat(3) = s0hh
END SUBROUTINE ALLBACKSCATTER_OPERATOR_1GEOM_FWV
...
! local declarations
  REAL(kind=8) :: s0hh
  REAL(kind=8), DIMENSION(nbdirsmax) :: s0hh_fwv
  INTEGER :: nbdirs
...
!:
!   variations   of useful results: s0hh s0vh s0vv
!   with respect to varying inputs: lai lai_coeff canht mv
!***********************************************************
!     sigma0_vhvvhh
!
...
!-- sum-up contributions
    s0hh_fwv(nd) = s0c_fwv(nd, 1) + s0g_fwv(nd, 1) + s0cgt_fwv(nd, 1) + &
&     s0gcg_fwv(nd, 1)
