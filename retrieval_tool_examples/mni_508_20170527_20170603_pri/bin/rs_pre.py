...
class RetrievalSetup(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.generic_prior     = [1.5, 1.5, 0.2] #LAI[m2/m2], canht[m], SM[m3/m3]
...
def pre_synthetic(options):
    ...
    if len(options.mission_lst)==0 and options.target_select==None and options.target_schedule==None:
        msg = "For synthetic mode at least one mission and/or extra target time points must be specified. None of those were specified on invocation, cannot continue!"
...
def create_argument_parser(progname=None):
    ...
    parser = argparse.ArgumentParser( prog=os.path.basename(progname), usage=globals()['__doc__'] )
    ...
    xparser = subparsers.add_parser( 'pre_synthetic',
                                     help="""Generation of all necessary input files for operating the retrieval system in synthetic mode. To that purpose, the user needs to supply (via the command line) at least one mission from S1A/S1B/S2A/S2B and/or explicitly select target extra time points. Synthetic experiments are being prepared for the Munich test-site. Sentinel-1/Sentinel-2 overpass times and a JULES state vector are available for this site and for year 2017. By default the synthetic experiment will be prepared for the period {} - {}.""".format(
                                         TIME_START_SYN.strftime('%Y%m%d'), TIME_END_SYN.strftime('%Y%m%d')) )
    #-- add common options
...
#    _add_output_options(xparser)
    ...
    xparser = subparsers.add_parser( 'pre_general',
                                     help="""Generation of all necessary input files for application of the prototype retrieval system on observation and state information provided by the user. The user has numerous options to configure the tool for the actual retrieval, which can be listed by invoking '{} pre_general -h'.""".format(progname) )
    #-- add common options
    _add_common_options(xparser)
    xparser.add_argument( '--site_nml',
...
if __name__ == '__main__':
    ...
    #
    parser = create_argument_parser(progname=os.path.abspath(__file__))
